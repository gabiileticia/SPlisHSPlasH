...
def main():
    ...
    base.setValueFloat(base.DATA_EXPORT_FPS, 15.0)
    ...
    # scene.materials.append(Scenes.MaterialData(id='Fluid', emitterReuseParticles=True,emitterBoxMin= [-24.0,-5.5,-0.35],emitterBoxMax=[25.0,5.5,0.35]))
    ...
    # scene.boundaryModels.append(Scenes.BoundaryData(meshFile="../models/UnitBox.obj", translation=[25., -3.0, 0.], scale=[100, 1.0, 0.35], color=[0.1, 0.4, 0.5, 1.0], mapThickness=1.0, isWall=True, mapInvert=True, mapResolution=[25, 25, 25], isDynamic=False))
    scene.boundaryModels.append(Scenes.BoundaryData(meshFile="../models/UnitBox.obj", translation=[0, 0.0, 0.], scale=[50, 10.0, 0.35], color=[0.1, 0.4, 0.5, 1.0], mapThickness=1.0, isWall=True, mapInvert=False, mapResolution=[25, 25, 25], isDynamic=False))
    scene.emitters.append(Scenes.EmitterData(id='Fluid', x=[-24,0,0], width=8, height=120, axis = [0,1,0], angle = 0, emitStartTime=0.0, emitEndTime=60.0, velocity=1.0,maxEmitterParticles=10000))
