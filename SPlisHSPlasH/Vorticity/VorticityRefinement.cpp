...
int VorticityRefinement::VORTICITY_FLAG = -1;
int VorticityRefinement::VISCOSITY_VORT_REF = -1;
int VorticityRefinement::KREY = -1;
int VorticityRefinement::BASEREY = -1;
int VorticityRefinement::MOD_FLAG = -1;
	...
	VorticityBase(model)
{
    ...
    m_vorticity_final_dissipation.resize(model->numParticles(), Vector3r::Zero());
    ...
    m_velocity_advected.resize(model->numParticles(), Vector3r::Zero());
    m_acceleration_vr.resize(model->numParticles(), Vector3r::Zero());
    m_position_advected.resize(model->numParticles(), Vector3r::Zero());
    ...
    m_lambdatwo.resize(model->numParticles(), 0.0);
    m_is_vortex.resize(model->numParticles(), 0.0);
    ...
    m_vorticity_rate_laplacian_new.resize(model->numParticles(), Vector3r::Zero());
    m_direction_velocity.resize(model->numParticles(), 0.0);
    rey.resize(model->numParticles(), 0.0);

    m_flag = static_cast<Real>(1.0);
    flag_mod = static_cast<Real>(1.0);
    m_vorticity_refinement_alpha = static_cast<Real>(1.0);
    m_v_v = static_cast<Real>(0.05);
    krey = static_cast<Real>(3.0);
    reybase = static_cast<Real>(0.2);
    ...
    model->addField({ "vorticity_final_dissipation", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &m_vorticity_final_dissipation[i][0]; }, true });
    ...
    model->addField({ "m_vorticity_rate_laplacian_new", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &m_vorticity_rate_laplacian_new[i][0]; }, true });
    ...
    model->addField({ "m_lambdatwo", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &m_lambdatwo[i]; }, true });
    model->addField({ "m_is_vortex", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &m_is_vortex[i]; }, true });
	model->addField({ "m_direction_velocity", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &m_direction_velocity[i]; }, true });
    model->addField({ "rey", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &rey[i]; }, true });
...
VorticityRefinement::~VorticityRefinement(void)
{
    ...
    m_model->removeFieldByName("vorticity_final_dissipation");
    ...
    m_model->removeFieldByName("m_lambdatwo");
    m_model->removeFieldByName("m_is_vortex");
    m_model->removeFieldByName("m_direction_velocity");
    m_model->removeFieldByName("rey");
    ...
    m_vorticity_final_dissipation.clear();
    m_stream.clear();
...
void VorticityRefinement::initParameters()
{
    ...
    VISCOSITY_VORT_REF = createNumericParameter("viscosityVR", "Viscosity Coefficient for VR", &m_v_v);
    setGroup(VISCOSITY_VORT_REF, "Fluid Model|Vorticity");
    setDescription(VISCOSITY_VORT_REF, "Viscosity coeff for VR. Controls the amount of viscosity on derivative.");
    rparam = static_cast<RealParameter*>(getParameter(VISCOSITY_VORT_REF));

    VORTICITY_FLAG = createNumericParameter("accelXvelocity", "Acceleration or Velocity", &m_flag);
    setGroup(VORTICITY_FLAG, "Fluid Model|Vorticity");
    rparam = static_cast<RealParameter*>(getParameter(VORTICITY_FLAG));

    MOD_FLAG = createNumericParameter("Mod visc", "Mod visc", &flag_mod);
    setGroup(MOD_FLAG, "Fluid Model|Vorticity");
    rparam = static_cast<RealParameter*>(getParameter(MOD_FLAG));

    KREY = createNumericParameter("k rey", "k rey", &krey);
    setGroup(KREY, "Fluid Model|Vorticity");
    rparam = static_cast<RealParameter*>(getParameter(KREY));

    BASEREY = createNumericParameter("rey base", "rey base", &reybase);
    setGroup(BASEREY, "Fluid Model|Vorticity");
    rparam = static_cast<RealParameter*>(getParameter(BASEREY));
...
void VorticityRefinement::step()
{
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            //compute linear field vorticity based on velocity from dfsph on current timestep
            //Compute vorticity derivative based on vorticity of last step. Using position and velocity of current step
            ...
            m_acceleration_vr[i] = m_model->getAcceleration(i);
            /////////////////////////////////////////////
            ...
            forall_fluid_neighbors(
                ...
                const Real mass_j = fm_neighbor->getMass(neighborIndex);

                const Vector3r xij = xi - xj;
                const Vector3r gradW = sim->gradW(xij);

                vorticity_linear_field += (mass_j / density_i) * (vi - vj).cross(gradW);
            );
            ...
            if (t < 0.002) {
                m_vorticity_init[i] = vorticity_linear_field;
                m_vorticity_corrected_end[i] = vorticity_linear_field;
            }
    ...
    #pragma omp for schedule(static)  
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            forall_fluid_neighbors(
                ...
                if (sim->is2DSimulation()) {
                    vorticity_rate_laplacian.z() += 2 * (d + 2) * (mass_j / density_j) * vort_ij.z() * xij.dot(gradW) / (xij.dot(xij) + 0.01 * h2);
                }
                else {
                    vorticity_rate_laplacian += 2 * (d + 2) * (mass_j / density_j) * ((vort_ij.dot(xij)) / (xij.dot(xij) + 0.01 * h2)) * gradW;
                }
            );
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            forall_fluid_neighbors(
                ...
                const Vector3r xij = xi - xj;
                const Real density_j = fm_neighbor->getDensity(neighborIndex);
                //const Real density_j = m_model->getDensity(i);
                const Real mass_j = fm_neighbor->getMass(neighborIndex);
        ...
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            forall_fluid_neighbors(
                ...
                // compute delta velocity
                delta_velocity += (mass_j / density_i) * (stream_i - stream_j).cross(gradW);
                //delta_velocity -= density_i * mass_j * (stream_i / (density_i * density_i) + stream_j / (density_j * density_j)).cross(gradW);
            );
        ...
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            forall_fluid_neighbors(
                ...
                const Vector3r xij = xi - xj;
                const Vector3r gradW = sim->gradW(xij);
                //vorticity through linear fieldss
                if (m_flag == 1.0) {
                    vorticity_corrected_end += (mass_j / density_i) * (vi - vj).cross(gradW);
                    //vorticity_corrected_end -= density_i * mass_j * (vi / (density_i * density_i) + vj / (density_j * density_j)).cross(gradW);
                }
                else {
                    vorticity_corrected_end += (mass_j / density_i) * ((vi + vorticity_refinement_alpha * m_delta_velocity[i]) - (vj + vorticity_refinement_alpha * m_delta_velocity[neighborIndex])).cross(gradW);
                    //vorticity_corrected_end -= density_i * mass_j * ((vi + vorticity_refinement_alpha * m_delta_velocity[i]) / (density_i * density_i) + (vj + vorticity_refinement_alpha * m_delta_velocity[neighborIndex]) / (density_j * density_j)).cross(gradW);
                }
                    );
                ...
                m_vorticity_final_dissipation[i] = m_vorticity_equation[i] - vorticity_corrected_end;
        }
        ...
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            S.row(0) = (m_gradV_x[i] + Vector3r(m_gradV_x[i][0], m_gradV_y[i][0], m_gradV_z[i][0])) / 2;
            S.row(1) = (m_gradV_y[i] + Vector3r(m_gradV_x[i][1], m_gradV_y[i][1], m_gradV_z[i][1])) / 2;
            S.row(2) = (m_gradV_z[i] + Vector3r(m_gradV_x[i][2], m_gradV_y[i][2], m_gradV_z[i][2])) / 2;

            Eigen::Matrix<Real, 3, 3>  O;
            O.row(0) = (m_gradV_x[i] - Vector3r(m_gradV_x[i][0], m_gradV_y[i][0], m_gradV_z[i][0])) / 2;
            O.row(1) = (m_gradV_y[i] - Vector3r(m_gradV_x[i][1], m_gradV_y[i][1], m_gradV_z[i][1])) / 2;
            O.row(2) = (m_gradV_z[i] - Vector3r(m_gradV_x[i][2], m_gradV_y[i][2], m_gradV_z[i][2])) / 2;

            Eigen::Matrix<Real, 3, 3>  A = S * S + O * O;
            Eigen::EigenSolver<Eigen::Matrix<Real, 3, 3>> solver(A);

            Vector3r v = solver.eigenvalues().real();
            std::array<double, 3> arr = { v(0), v(1), v(2) };
...
void SPH::VorticityRefinement::performNeighborhoodSearchSort()
{
    ...
    // d.sort_field(&m_velocity_from_dfsph[0]);
     ...
     d.sort_field(&m_acceleration_vr[0]);
