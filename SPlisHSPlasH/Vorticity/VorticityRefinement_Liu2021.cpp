	...
	NonPressureForceBase(model)
{
	...
	model->addField({ "dissipated vorticity", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &m_dissipatedVorticity[i][0]; }, true });
	model->addField({ "stream function", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &m_streamFunction[i][0]; }, true });
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < static_cast<int>(numParticles); i++)
		{
			...
			if (m_model->getParticleState(i) == ParticleState::Active)
				ai += invDt * m_vorticityCoeff * delta_vi;
		}
